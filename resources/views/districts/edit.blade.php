<x-layouts.app.sidebar title="Edit Kecamatan">
    <flux:main>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="kemendagri_code" id="kemendagri_code" 
                                   value="{{ old('kemendagri_code', $district->kemendagri_code) }}"
                        ...
                        <div>
                            ...
                            <select name="province_id" id="province_id" 
                                ...
                                @foreach($provinces as $province)
                                    <option value="{{ $province->id }}" {{ old('province_id', $selectedProvinceId) == $province->id ? 'selected' : '' }}>
                                        {{ $province->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="city_id" id="city_id" 
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm @error('city_id') border-red-500 @enderror">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    provinceSelect.addEventListener('change', function() {
        ...
        // Clear city options except the current one
        const currentCityId = citySelect.value;
        citySelect.innerHTML = '<option value="">Pilih Kota/Kabupaten</option>';
        
        if (provinceId) {
            // Fetch cities for selected province
            fetch(`/districts/cities?province_id=${provinceId}`)
