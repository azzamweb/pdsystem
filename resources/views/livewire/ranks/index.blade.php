<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Data Pangkat</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola data pangkat dalam sistem</p>
        </div>
    ...
    <div class="flex items-center space-x-4">
        <div class="flex-1 relative">
            ...
            <input 
                ...
                placeholder="Cari pangkat (kode, nama)..."
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                    @if($rank->users_count > 0) bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300
                                    @else bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 @endif">
                                    {{ $rank->users_count }} Pegawai
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <button 
                                        wire:click="delete({{ $rank->id }})"
                                        wire:confirm="Apakah Anda yakin ingin menghapus pangkat {{ $rank->fullName() }}?"
                                        class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 p-1 rounded"
                                        title="Hapus"
                                        @if($rank->users_count > 0) disabled @endif
                                    >
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                @if($search)
                                    Tidak ada pangkat yang ditemukan dengan kata kunci "{{ $search }}"
                                @else
